...
public class AuthController {
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestBody AuthenticationRequest authenticationRequest) throws Exception {
            System.out.println("--- AuthController.createAuthenticationToken ---");
    System.out.println("Received login request for email: " + authenticationRequest.getEmail());
    System.out.println("Received password: " + authenticationRequest.getPassword());
    System.out.println("------------------------------------------");

        try {
            authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(authenticationRequest.getEmail(), authenticationRequest.getPassword()) // <-- MODIFICATO
            );
        } catch (BadCredentialsException e) {
            System.err.println("Authentication failed! Exception: " + e.getClass().getName());
        System.err.println("Error message: " + e.getMessage());
        System.err.println("---------------------------------------------");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Email o password errati"); // Messaggio più chiaro
        }

        // Anche qui, usa .getEmail() per coerenza
        final UserDetails userDetails = userService.loadUserByUsername(authenticationRequest.getEmail()); // <-- MODIFICATO
